...
void actualizaTiempo(int segundos)
{
    ...
    HORA_TIVA.segundo = HORA_TIVA.segundo + segundos;

    if( HORA_TIVA.segundo >= 60 )
    {
        HORA_TIVA.segundo = HORA_TIVA.segundo - 60 ;

        if (++HORA_TIVA.minuto >= 60)
        {
            HORA_TIVA.minuto = 0 ;

            if (++HORA_TIVA.hora == 24)
                {
                HORA_TIVA.hora = 0;
                }
        }
    ...
    strcpy(HORA_TIVA.text,"");
    if ( HORA_TIVA.hora < 10 )   strcat(HORA_TIVA.text,"0");

    itoa(HORA_TIVA.hora, aux);
    strcat(HORA_TIVA.text,aux);
    strcat(HORA_TIVA.text,":");

    if( HORA_TIVA.minuto < 10 )   strcat(HORA_TIVA.text,"0") ;

    itoa(HORA_TIVA.minuto, aux);
    strcat(HORA_TIVA.text,aux);
    strcat(HORA_TIVA.text,":");

    if ( HORA_TIVA.segundo < 10 )   strcat(HORA_TIVA.text,"0") ;
    itoa(HORA_TIVA.segundo, aux);

    strcat(HORA_TIVA.text,aux);
    strcat(HORA_TIVA.text,"\0");
